...
| A35 | 🔴 Dashboard "main page takes very long to load" — stuck on the busy spinner, tables show "No data" | The dashboard paged the **whole** `EmployeeDq` (~41k) **and** the whole `DataQualityIssue` (~60–90k) — ~25 sequential `$skip`/`$top` requests. Since A34 both are `GROUP BY` views over the 31-branch `ZI_HR360_ISSUE` union, so **every page re-runs the full union + group-by + sort** before `OFFSET` — deep pagination of an aggregating view is ~O(n²) and gets slower each page. | (a) `ZI_HR360_EMP_KPI` gains **`FailedChecks`** = `string_agg( Iss.CheckID, ',' )` — the comma list of the employee's failed checks, built server-side in the one group-by it already does. (b) `ZC_HR360_EMP_DQ` exposes it. (c) The dashboard now reads **only `/EmployeeDq`** (one big request, `$select` incl. `FailedChecks`, no `$count`, page size 20k) and splits `FailedChecks` client-side — it never touches the `DataQualityIssue` union. ~130k rows / 25 requests → ~41k rows / 1–3 requests. **Watch:** if `string_agg` doesn't activate on this release, fall back to a 2×`int4` failure bitmask on `EMP_KPI`. | v0.44 |
