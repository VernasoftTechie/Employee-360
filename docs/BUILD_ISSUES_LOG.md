...
| A27 | 🔴 **Fiori/OData runtime**: "Property 'EducationType' has the same EDM name as entity type 'EducationType'" | OData V4 names an entity **type** as `<EntitySet>Type`. Entity set `Education` → entity type `EducationType`. Our view also had a **property** literally named `EducationType` → name clash in the EDM. Same trap for every `<EntitySetSingular>Type` property: `QualificationType` (set `Qualification`), `DocumentType` (set `Document`), `AttendanceType` (set `Attendance`). | Renamed those elements to `…TypeCode` in the `ZI_` interface views (+ the `ZC_` projections, `@UI`, test class). **Rule: never name a CDS element `<X>Type` when `<X>` is (or could be) an OData entity-set name.** | v0.23 |
| A28 | 🔴 **OData runtime**: `ABAP Runtime error 'RAISE_SHORTDUMP'` / `$batch failed` when `ZC_HR360_KPI_OVERVIEW` is filtered (e.g. CompanyCode = 1000) | The view aggregated `count`/`sum`/`avg` **on top of `ZI_HR360_EMPLOYEE`**, which itself joins `ZI_HR360_EMP_KPI` (a `group by` over the 12-branch `ZI_HR360_ISSUE` union) plus `_PERSONAL` + `_HIREDATE`. Aggregate-over-aggregate-over-union across a whole company code generated an SQL the runtime could not execute → dump. | Aggregate straight from the flat `ZI_HR360_EMP_BASIC` ⋈ `ZI_HR360_EMP_KPI` join (the same shape the executable reports use and that runs fine); compute `QualityStatus` inline; `avg( division(...) as abap.dec( 16, 2 ) )` with a wide accumulator. | v0.26 |

## G. Repo hygiene (cont.)

| G4 | 🔴 abapGit pull **still** shows every DDLS as "Delete and recreate local object" on every pull (after the BOM fix) | abapGit's DDLS serializer also writes a **`.ddls.baseinfo`** file (JSON: `FROM` = every `select from` / `join` source, `ASSOCIATED` = association targets, `BASE` for projections). The repo had none, so on every pull abapGit's local re-serialization produced a `.baseinfo` that wasn't in the repo → object flagged different → DDLS gets drop+recreate. | Generated a best-effort `.baseinfo` for all 28 DDLS. **Definitive fix: after the next pull, do abapGit Stage → Commit (push) *from the SAP system*** — that writes abapGit's own exact serialization (baseinfo, xml field order, everything) back to the repo, and every pull after that is clean. | v0.26 |
| A29 | 🔴 "Unexpected keyword CASE" — `ZC_HR360_KPI_OVERVIEW` won't activate (v0.26) | A `CASE … END` expression used **as a `key` element** and repeated in the `GROUP BY` clause. CDS won't take a bare CASE there. | Compute `QualityStatus` + `QualityStatusCriticality` + `CompletenessPercent` once, in the flat `ZI_HR360_EMP_KPI` view; the aggregating view then references `k.QualityStatus` — a plain field that groups cleanly. | v0.27 |
| A30 | 🔴 Service binding: "Entity type ZC_HR360_DQ_BYCHECK / ZC_HR360_KPI_OVERVIEW has no key field assigned" + app "Entity type … does not have a key" | An `@Analytics.query` view has **no `key` fields** (the OLAP engine handles identity). An **OData V4 – UI** service binding requires every exposed entity type to have a key. | Reverted the two dashboard views to plain **aggregating views with `key` on every dimension + `GROUP BY`** (the v0.27 form — activates, returns data, no dump). Added `@Aggregation.default` on the measures. Chart rendering now belongs to a proper **Overview Page / Analytical List Page app built in BAS**, not the plain ADT List Report preview. | v0.29 |
| A31 | 🔴 "Unexpected keyword CAST" — `ZC_HR360_KPI_HEADLINE` won't activate | Same shape as A29: a single grand-total row needed `group by cast( 'HR360' as abap.char( 5 ) )` — a `cast()` **expression is not allowed in `GROUP BY`** (only element names / plain columns). | Dropped the single-row headline view. The dashboard KPI number tiles are OVP cards over `StatusSplit` (filtered to a status) and `CheckFailure`; a true 1-row "totals" view, if needed, adds a constant grouping column to `ZI_HR360_EMP_KPI` and groups by that field. | v0.31 |
| A32 | 🔴 **Fiori preview**: "Error while processing building block FilterBar / Trailing text was found in the XML" — Employee List Report stopped rendering | Over-ambitious `@UI` on `ZC_HR360_EMPLOYEE`: a V2-style `@UI.presentationVariant` (`visualizations: [{ type: #AS_LINEITEM }]`), `@UI.dataPoint` + `@UI.lineItem: [{ type: #AS_DATAPOINT }]` progress cells, and `#DATAPOINT_REFERENCE` header facets with `targetQualifier`. One of these produced malformed EDM the FE FilterBar building block couldn't template. | Stripped `ZC_HR360_EMPLOYEE` (and the 4 dashboard views) to a **minimal, known-good** `@UI` set: `headerInfo` + `lineItem` + `selectionField` + `identification` + `LINEITEM_REFERENCE` facets + `criticality` colouring only. The completeness gauge / header KPI datapoints are for the **UI team** to add in a metadata extension against a live system where they can test each annotation. | v0.33 |

**Rule:** keep CDS `@UI` to the four basics (`lineItem` / `selectionField` /
...
| U6 | 🟡 Dashboard didn't feel as compact/polished as the Control Tower dashboard; the org-metric SegmentedButton (`% Critical / # Critical / Avg complete…`) overflowed its card and the third option was cut off; no always-available link to Employee 360 (the "View employees" button only showed after drilling in). | The dashboard used `sap.viz.VizFrame` charts (heavy, hard to size tight). | **v0.43 redesign** — dropped `sap.viz` / `sap.f` / `sap.ui.layout` entirely; charts are now plain SVG/CSS emitted by the controller into `core:HTML` blocks (same technique as `VS-Tower/ui/controltower`): compact KPI tiles, a small 3-status donut, severity-coloured check bars, and **stacked critical/warning/ok bars per org node**. The org-metric toggle became a 2-option `sap.m.Select` ("Highest critical %" / "Largest headcount"). **"Open in Employee 360" is now always visible** in the org section toolbar and carries the current drill scope; clicking an org bar drills (or, at leaf level, opens that unit's filtered list). One delegated click listener on the view root handles the `[data-drill]` elements. | v0.43 |

| A35 | 🔴 Dashboard "main page takes very long to load" — stuck on the busy spinner, tables show "No data" | The dashboard paged the **whole** `EmployeeDq` (~41k) **and** the whole `DataQualityIssue` (~60–90k) — ~25 sequential `$skip`/`$top` requests. Since A34 both are `GROUP BY` views over the 31-branch `ZI_HR360_ISSUE` union, so **every page re-runs the full union + group-by + sort** before `OFFSET` — deep pagination of an aggregating view is ~O(n²) and gets slower each page. | v0.44 tried `string_agg( Iss.CheckID, ',' )` in `ZI_HR360_EMP_KPI` → **"column CHECKID is not contained in the GROUP BY list"**: `string_agg` is **not recognised as an aggregate function on this release** (7.58) inside a view entity. **v0.45** replaced it with `FailureBitmask` = `cast( sum( case Iss.CheckID when 'MAND_DOB' then 1 when 'INVALID_DOB' then 2 … when 'DOC_NONE' then 1073741824 else 0 end ) as abap.int4 )` — bit *i* = 2^*i*, one bit per check, **order pinned to `checkCatalogue.json`'s array order** (a comment says so in both files). `ZC_HR360_EMP_DQ` exposes it; the dashboard reads **only `/EmployeeDq`** (one request, `$select` incl. `FailureBitmask`, no `$count`) and decodes it client-side (`bm & catalogue[i].bit`, bit *i* = `Math.pow(2,i)`). Max mask = 2^31−1 = int4 max (a completely broken employee), still a valid positive int4 and a valid JS int32 for `&`. ~130k rows / 25 requests → ~41k rows / 1–3 requests. | v0.44 → v0.45 |

**Rule:** `string_agg` is unavailable in CDS view entities on this system — for a
