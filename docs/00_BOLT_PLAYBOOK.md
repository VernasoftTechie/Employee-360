...
| G15 | abapGit rejects a hand-authored serialization for an unfamiliar object type that carries a binary/layout payload (e.g. SFPF Adobe Form) even though the content is a genuine, confirmed-authentic export from the system | modeled the file on a raw SAP GUI "Utilities > Download" export (bare `<asx:abap>` root, payload base64-embedded inline) instead of abapGit's own serializer output for that object type (`LCL_OBJECT_<TYPE>`) — the two are different shapes for the same underlying data, and only one is what abapGit actually reads. abapGit's SFPF serializer, for example, wraps everything in `<abapGit version="..." serializer="LCL_OBJECT_SFPF" ...>` and ships the layout as a **separate companion file** (`<name>.sfpf.xdp`), not embedded | don't keep guessing the wrapper shape from a non-abapGit export. Create the simplest possible object of that exact type in the system, commit it, `git pull`, and copy its outer structure exactly — content facts (field names/types) can still come from other sources, but the wrapper/file-split shape can't be reliably reverse-engineered from a different export mechanism |
...
| D8 | "The variable `<X>` must be escaped using `@`" on a SELECT target | modern strict Open SQL requires **every** host variable — an existing field symbol/variable, not just an inline `DATA()` — to be `@`-prefixed as a SELECT source/target |
| D9 | ATC "ABAP Doc comment is in the wrong position" that a blank line alone doesn't fix | ABAP Doc (`"!`) doesn't support commenting a compound `TYPES: BEGIN OF … END OF …` chain declaring more than one type — downgrade to a plain `"` comment there; `"!` is fine before `METHODS`/`DATA`/single-type declarations |
